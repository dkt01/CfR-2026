...
+/-1, and the log std is held inside `log_std_range`.  The deterministic
action is the mean itself; export_policy.py writes `output: tanh` so the
car's NumpyPolicy squashes it the same way.
"""
...
class SquashedMeanPolicy(ActorCriticPolicy):
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor):
        ...
        log_std = self.log_std.clamp(*self.log_std_range)
