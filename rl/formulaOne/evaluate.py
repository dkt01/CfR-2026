...
def table(name, rows, graze_margin):
    ...
    times = (
        np.array([r["race_time"] for r in finished]) if finished else np.array([np.nan])
    )
    ...
    stops = (
        np.array([r["stop_distance"] for r in finished])
        if finished
    ...
    return dict(
        finish=len(finished) / len(rows),
        time=float(np.nanmean(times)),
        cte=float(np.mean(cte)),
...
def plot(path, track, log, cfg, label):
    ...
    hp = track.hairpin
    ax.scatter(
        track.x[hp], track.y[hp], s=3, color="0.6", zorder=2, label="hairpin (2.5 m/s)"
    )
    ...
    ax.set_title(
        f"{label} -- {elapsed[-1]:.2f} s for {cfg['env']['laps']} laps, "
    ...
    ax.plot(
        track.s,
        feasible_profile(track, cfg),
        color="steelblue",
        lw=1.0,
        ls="--",
        label="coast-feasible ideal",
    )
    ...
    ax.plot(station, steer, lw=0.7, color="darkorange", alpha=0.8, label="steering cmd")
    ax.axhline(0.0, color="0.5", lw=0.8)
    ax.set_xlabel("station (m)")
    ax.set_ylabel("m  /  command")
    ax.grid(alpha=0.25)
    ax.legend(fontsize=7)
    ax.set_title(
        f"cte {np.abs(lateral).mean():.3f} m mean, {np.abs(lateral).max():.3f} m worst",
        fontsize=8,
    )
...
def main():
    ...
    for name, deterministic in (("nominal", True), ("field", False)):
        ...
        results[name] = table(
            name,
            run(env, act, args.episodes if not deterministic else 64),
