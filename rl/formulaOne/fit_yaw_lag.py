...
def model_trace(cfg, speed, command, t_pre, t_post, yaw_tau=None,
                steering_tau=None):
    ...
    for path in paths:
        ...
        for key in d.files:
            ...
            if abs(steady) < 0.2 or np.sign(steady) != np.sign(cmd):
                print(f"  skipping {Path(path).name} cmd {cmd:+.2f}: "
                      f"steady yaw rate {steady:+.3f} rad/s is not a turn")
                continue
            out.append(dict(path=Path(path).name, speed=speed, cmd=cmd,
                            t=tg, rate=rate, pre=pre, post=post, steady=steady))
    return out
...
def rms(traces, cfg, **override):
    ...
    errs = []
    for tr in traces:
        tm, rm = model_trace(cfg, tr["speed"], tr["cmd"], tr["pre"], tr["post"],
                             **override)
...
def main():
    ...
    ap.add_argument("traces", type=Path, nargs="+", help="npz from measure_step_steer.py --save")
    ...
    print(f"\n  {len(traces)} traces, "
          f"{sorted({t['speed'] for t in traces})} m/s\n")
    ...
    print(f"  lag on the SERVO     (steering_tau {s_best:.2f})  "
          f"err {min(s_scores):.4f}")
    print(f"  lag on the CHASSIS   (yaw_tau {best_tau:.2f})      "
    ...
    for tr, e0, e1, tau in zip(traces, per0, per, per_tau):
        print(f"  {tr['speed']:5.1f} m/s cmd {tr['cmd']:+.2f}   "
    ...
    if args.write:
        ...
        import re
        new = re.sub(r"(  yaw_response_tau: )[0-9.]+(\s*#[^\n]*)?",
                     rf"\g<1>{best_tau:.2f}"
        ...
        print(f"\n  set  plant.yaw_response_tau: {best_tau:.2f}  "
              f"(or re-run with --write)")
