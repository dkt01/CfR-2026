...
def main():
    ...
    check(
        "min radius above the car's own",
        1 / np.abs(t.kappa).max() > 0.8,
        f"{1 / np.abs(t.kappa).max():.2f} m",
    )
    corridor = t.half_left + t.half_right
    check(
        "corridor never below the car's width",
        corridor.min() > cfg["vehicle"]["width"],
        f"{corridor.min():.2f} m min vs {cfg['vehicle']['width']} m car",
    )
    check(
        "cap honours the hairpin rule exactly",
        np.allclose(t.v_cap[t.hairpin], tc["v_hairpin"]),
        f"{tc['v_hairpin']} m/s over {100 * t.hairpin.mean():.0f}% of the lap",
    )
    check(
        "cap never exceeds the straight rule",
        t.v_cap.max() <= tc["v_straight"] + 1e-9,
        f"max {t.v_cap.max():.2f} m/s",
    )
    check(
        "start line is on the course",
        t.clearance(
            np.array([tc["vehicle_start"][0]]), np.array([tc["vehicle_start"][1]])
        )[0]
        > 0.3,
    )
    ...
    check(
        "no brakes: 5.2 -> 2.5 m/s takes ~9 m of track",
        8.0 < dist < 11.0,
    ...
    check(
        "acceleration is bridge-slew limited, not plant limited",
        1.4 < p2.speed[0] < 2.1,
        f"{p2.speed[0]:.2f} m/s one second in",
    )
    ...
    p3 = Plant(flat, n, rng)
    p3.reset(
        np.ones(n, dtype=bool), np.zeros(n), np.zeros(n), np.zeros(n), np.full(n, 2.0)
    )
    ...
    kinematic_r = (
        cfg["plant"]["wheelbase"]
        + cfg["plant"]["understeer_gradient"] * p3.speed[0] ** 2
    ) / np.tan(angle)
    check(
        "tire scrub widens the radius by the measured ~1.10x",
    ...
    check(
        "kept some clearance even at the floor",
    ...
    check(
        "steering is not jittery",
        result.get("steer_jerk_rms", 9) < 0.05,
        f"rms change in steering step {result.get('steer_jerk_rms', 0):.4f} per tick",
    )
    ...
    for i in range(int(2.5 / dt)):
        rates.append(
            float(pl.substep(np.array([0.55]), np.array([2.5]), np.array([dt]))[0])
        )
    ...
    check(
        "takes as long as Gazebo to build yaw rate",
        0.45 < t_half < 0.75,
    ...
    print(
        f"             cte {result.get('mean_cte', 0):.3f} m mean / "
