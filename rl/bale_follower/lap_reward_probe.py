...
def scenarios(config: LapRewardConfig, dt: float, top_speed: float):
    ...
    states = [
        ("racing, clear, top speed", dict(
            delta_s=top_speed * dt, dt=dt, speed=top_speed, clearance=clear,
            steer_rate_fraction=0.0, steer_fraction=0.1, prev_steer_fraction=0.1,
            collided=False)),
        ("hairpin, 1.9 m/s, held lock", dict(
            delta_s=hairpin_speed * dt, dt=dt, speed=hairpin_speed, clearance=0.20,
            steer_rate_fraction=0.05, steer_fraction=0.95, prev_steer_fraction=0.95,
            collided=False)),
        ("turn-in, full servo rate", dict(
            delta_s=3.0 * dt, dt=dt, speed=3.0, clearance=0.28,
            steer_rate_fraction=1.0, steer_fraction=0.5, prev_steer_fraction=0.06,
            collided=False)),
        ("sawing at top speed", dict(
            delta_s=top_speed * dt, dt=dt, speed=top_speed, clearance=clear,
            steer_rate_fraction=1.0, steer_fraction=0.4, prev_steer_fraction=-0.4,
            collided=False)),
        ("drifting off the line", dict(
            delta_s=top_speed * dt, dt=dt, speed=top_speed, clearance=clear,
            steer_rate_fraction=0.0, steer_fraction=0.1, prev_steer_fraction=0.1,
            collided=False, lateral=0.30, prev_lateral=0.20,
            heading_error=0.25, prev_heading_error=0.20)),
        ("coming back to the line", dict(
            delta_s=top_speed * dt, dt=dt, speed=top_speed, clearance=clear,
            steer_rate_fraction=0.0, steer_fraction=0.1, prev_steer_fraction=0.1,
            collided=False, lateral=0.20, prev_lateral=0.30,
            heading_error=0.20, prev_heading_error=0.25)),
        ("crawling, clear", dict(
            delta_s=0.5 * dt, dt=dt, speed=0.5, clearance=clear,
            steer_rate_fraction=0.0, steer_fraction=0.0, prev_steer_fraction=0.0,
            collided=False)),
        ("stopped, clear", dict(
            delta_s=0.0, dt=dt, speed=0.0, clearance=clear,
            steer_rate_fraction=0.0, steer_fraction=0.0, prev_steer_fraction=0.0,
            collided=False)),
        ("top speed, brushing a bale", dict(
            delta_s=top_speed * dt, dt=dt, speed=top_speed, clearance=0.05,
            steer_rate_fraction=0.0, steer_fraction=0.1, prev_steer_fraction=0.1,
            collided=False)),
        ("top speed, 0.15 m gap", dict(
            delta_s=top_speed * dt, dt=dt, speed=top_speed, clearance=0.15,
            steer_rate_fraction=0.0, steer_fraction=0.1, prev_steer_fraction=0.1,
            collided=False)),
        ("reversing, NOT stuck", dict(
            delta_s=-1.2 * dt, dt=dt, speed=-1.2, clearance=clear,
            steer_rate_fraction=0.0, steer_fraction=0.0, prev_steer_fraction=0.0,
            collided=False)),
        ("rocking in place (the exploit)", dict(
            delta_s=0.0, dt=dt, speed=1.0, clearance=clear,
            steer_rate_fraction=0.3, prev_steer_rate_fraction=-0.3,
            steer_fraction=0.1, prev_steer_fraction=-0.1,
            collided=False, recovering=True,
            heading_error=0.4, prev_heading_error=0.4)),
        ("wedged, sitting still", dict(
            delta_s=0.0, dt=dt, speed=0.0, clearance=0.12,
            steer_rate_fraction=0.0, steer_fraction=1.0, prev_steer_fraction=1.0,
            collided=False, recovering=True,
            heading_error=1.2, prev_heading_error=1.2)),
        ("wedged, nose against a bale", dict(
            delta_s=0.0, dt=dt, speed=0.0, clearance=0.0,
            steer_rate_fraction=0.0, steer_fraction=1.0, prev_steer_fraction=1.0,
            collided=False, recovering=True,
            heading_error=1.2, prev_heading_error=1.2)),
        ("wedged, reversing and turning out", dict(
            delta_s=-1.2 * dt, dt=dt, speed=-1.2, clearance=0.18,
            steer_rate_fraction=0.2, steer_fraction=1.0, prev_steer_fraction=1.0,
            collided=False, recovering=True,
            heading_error=1.125, prev_heading_error=1.2)),
        ("collision", dict(
            delta_s=0.3 * dt, dt=dt, speed=0.3, clearance=0.0,
            steer_rate_fraction=0.5, steer_fraction=0.8, prev_steer_fraction=0.3,
            collided=True)),
    ]
    ...
    states.insert(4, ("chattering, top speed", dict(
        delta_s=top_speed * dt, dt=dt, speed=top_speed, clearance=clear,
        steer_rate_fraction=0.5, prev_steer_rate_fraction=-0.5,
        steer_fraction=0.05, prev_steer_fraction=-0.02, collided=False)))
    states.insert(4, ("smooth turn-in, same mean rate", dict(
        delta_s=top_speed * dt, dt=dt, speed=top_speed, clearance=clear,
        steer_rate_fraction=0.5, prev_steer_rate_fraction=0.5,
        steer_fraction=0.3, prev_steer_fraction=0.25, collided=False)))
    return states
...
def main() -> None:
    ...
    parser.add_argument("--set", action="append", default=[],
                        metavar="key=value", help="override one coefficient")
    ...
    columns = ["progress", "time", "stall", "clearance", "touch", "steering",
               "align", "shaping", "lap_bonus"]
    print(f"control {1 / dt:.0f} Hz (dt {dt * 1000:.0f} ms), "
          f"max_speed {top} m/s, loop {loop} m")
    print(f"break-even speed k_time/k_progress = "
          f"{config.k_time / config.k_progress:.2f} m/s\n")
    header = f"{'state':<32}" + "".join(f"{c[:8]:>9}" for c in columns) + \
             f"{'TOTAL':>9}{'per s':>9}"
    print(header)
    print("-" * len(header))
    results = {}
    ...
    for lap_time in (28.4, 30.0, 33.0, 37.0, 42.0, 50.0):
        bonus = compute_lap_reward(
            config, delta_s=0.0, dt=dt, speed=3.0, clearance=0.3,
            steer_rate_fraction=0.0, prev_steer_rate_fraction=0.0,
            steer_fraction=0.0, prev_steer_fraction=0.0,
            collided=False, lap_time=lap_time).lap_bonus
        dense = config.k_progress * loop - config.k_time * lap_time
        print(f"  {lap_time:>5.1f} s lap: dense {dense:>7.1f} + bonus "
    ...
    assert results["chattering, top speed"].total < \
    ...
    assert wedged_out > rocking, (
        "backing out must beat rocking in place")
    ...
    assert wedged_out > wedged_still, (
        "backing out and turning down the course must beat sitting in the "
        "wedge")
    ...
    fast = compute_lap_reward(config, delta_s=0.0, dt=dt, speed=3.0,
                              clearance=0.3, steer_rate_fraction=0.0,
                              prev_steer_rate_fraction=0.0,
                              steer_fraction=0.0, prev_steer_fraction=0.0,
                              collided=False, lap_time=30.0)
    slow = compute_lap_reward(config, delta_s=0.0, dt=dt, speed=3.0,
                              clearance=0.3, steer_rate_fraction=0.0,
                              prev_steer_rate_fraction=0.0,
                              steer_fraction=0.0, prev_steer_fraction=0.0,
                              collided=False, lap_time=40.0)
    # Shaping must be directional...
    assert results["coming back to the line"].total > \
    ...
    for _ in range(200):
        ...
        loop_total += compute_lap_reward(
            config, delta_s=0.0, dt=dt, speed=2.0, clearance=0.32,
            steer_rate_fraction=0.0, prev_steer_rate_fraction=0.0,
            steer_fraction=0.0, prev_steer_fraction=0.0, collided=False,
            lateral=nxt[0], prev_lateral=state[0],
            heading_error=nxt[1], prev_heading_error=state[1]).shaping
        state = nxt
    loop_total += compute_lap_reward(
        config, delta_s=0.0, dt=dt, speed=2.0, clearance=0.32,
        steer_rate_fraction=0.0, prev_steer_rate_fraction=0.0,
        steer_fraction=0.0, prev_steer_fraction=0.0, collided=False,
        lateral=0.0, prev_lateral=state[0],
        heading_error=0.0, prev_heading_error=state[1]).shaping
    ...
    print(f"shaping round a 201-step closed walk: {loop_total:+.3f}, "
