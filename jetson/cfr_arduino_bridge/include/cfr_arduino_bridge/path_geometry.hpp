...
namespace cfr_arduino_bridge {
  ...
  struct ControllerParams {
    ...
    double heading_deadband = 0.0;     ///< rad of yaw error ignored while holding a STRAIGHT heading
  ...
  /// Target pose for the active segment.  TURN segments target heading only,
  /// because their path radius comes from the vehicle's steering geometry.
  struct DesiredPose {
    Pose2D pose;
  ...
  /// Express @p pose in a local frame whose origin is @p reference_pose.
  Pose2D PoseRelativeTo(const Pose2D& pose, const Pose2D& reference_pose);

  /// Compute the desired endpoint for @p segment starting from @p start.
