...
struct Segment {
  ...
  double distance =
      0.0; ///< STRAIGHT: target distance, metres.  Negative reverses.
...
struct ControllerParams {
  ...
  double decel_distance =
      0.3; ///< m, begin slowing within this of the STRAIGHT target
  double decel_angle =
      0.3; ///< rad, begin slowing within this of the TURN target
  double min_creep_speed =
      0.08; ///< m/s floor while decelerating a STRAIGHT segment
  double min_turn_rate =
      0.15; ///< rad/s floor while decelerating a TURN segment
  double distance_tolerance = 0.03; ///< m, STRAIGHT segment is done within this
  double angle_tolerance =
      0.02; ///< rad (~1.1 deg), TURN segment is done within this
};
