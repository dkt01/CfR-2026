...
std::string Serialize(const JetsonToArduino &command) {
  ...
  const int written = std::snprintf(
      buffer.data(), buffer.size(), "%c,%03u,%03u\n",
      command.auto_ready ? '1' : '0', static_cast<unsigned>(command.steering),
...
bool Deserialize(const std::string &line, ArduinoToJetson &status) {
  ...
  uint8_t raw_mode = 0;
  if (!ParseBool(fields[0], parsed.estop) ||
      !ParseBool(fields[1], parsed.auto_arm) ||
      !ParseBool(fields[2], parsed.manual_start) ||
      !ParseUint8(fields[3], raw_mode) ||
