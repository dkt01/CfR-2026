...
public:
  PathFollowerNode() : rclcpp::Node("path_follower") {
    params_.cruise_speed =
        declare_parameter("cruise_speed", params_.cruise_speed);
    ...
    params_.decel_distance =
        declare_parameter("decel_distance", params_.decel_distance);
    params_.decel_angle = declare_parameter("decel_angle", params_.decel_angle);
    params_.min_creep_speed =
        declare_parameter("min_creep_speed", params_.min_creep_speed);
    params_.min_turn_rate =
        declare_parameter("min_turn_rate", params_.min_turn_rate);
    params_.distance_tolerance =
        declare_parameter("distance_tolerance", params_.distance_tolerance);
    params_.angle_tolerance =
        declare_parameter("angle_tolerance", params_.angle_tolerance);
    ...
    action_server_ = rclcpp_action::create_server<DrivePath>(
        ...
        [this](const rclcpp_action::GoalUUID &,
               std::shared_ptr<const DrivePath::Goal> goal) {
          return HandleGoal(goal);
        },
        [this](const std::shared_ptr<GoalHandleDrivePath> &goal_handle) {
    ...
    RCLCPP_INFO(get_logger(), "path_follower ready, action ~/drive_path, "
...
private:
  void OnOdom(const nav_msgs::msg::Odometry &msg) {
    ...
    current_pose_.yaw = YawFromQuaternion(q.w, q.x, q.y, q.z);
    have_odom_ = true;
    last_odom_time_ = now();
  }

  bool OdomStale() const {
    return !have_odom_ || (now() - last_odom_time_).seconds() > odom_timeout_;
  }

  rclcpp_action::GoalResponse
  HandleGoal(const std::shared_ptr<const DrivePath::Goal> &goal) {
    ...
    if (goal_pending_or_active_) {
      RCLCPP_WARN(get_logger(),
                  "rejecting goal, one is already pending or active");
    ...
    for (const auto &segment : goal->segments) {
      if (segment.type == cfr_interfaces::msg::PathSegment::TURN &&
          std::abs(segment.turn_angle) > kPi) {
        RCLCPP_WARN(get_logger(), "rejecting goal, a turn segment exceeds 180 "
  ...
  HandleCancel(const std::shared_ptr<GoalHandleDrivePath> & /*goal_handle*/) {
  ...
  void HandleAccepted(const std::shared_ptr<GoalHandleDrivePath> &goal_handle) {
    ...
    RCLCPP_INFO(get_logger(), "starting path with %zu segment(s)",
                segments_.size());
  }
  ...
  void OnTimer() {
    ...
    Segment segment;
    segment.type = ros_segment.type == cfr_interfaces::msg::PathSegment::TURN
                       ? SegmentType::kTurn
                       : SegmentType::kStraight;
    segment.distance = ros_segment.distance;
    segment.turn_angle = ros_segment.turn_angle;

    const SegmentCommand command = ComputeSegmentCommand(
        segment, segment_start_pose_, current_pose_, params_);
