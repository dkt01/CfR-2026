...
namespace cfr_arduino_bridge
{
  ...
  public:
    PathFollowerNode() : rclcpp::Node("path_follower")
    {
      ...
      RCLCPP_INFO(get_logger(), "path_follower ready, action ~/drive_path, waiting for odometry on ~/odom");
    }
  ...
  private:
    ...
    rclcpp_action::GoalResponse HandleGoal(const std::shared_ptr<const DrivePath::Goal> &goal)
    {
      ...
      for (const auto &segment : goal->segments)
      {
        ...
        {
          RCLCPP_WARN(get_logger(), "rejecting goal, a turn segment exceeds 180 degrees; split it into multiple");
          return rclcpp_action::GoalResponse::REJECT;
        }
      }
    ...
    void OnTimer()
    {
      ...
      {
        AbortGoal("segment did not complete within max_segment_duration, check for wheel slip or lost odometry");
        return;
      }
