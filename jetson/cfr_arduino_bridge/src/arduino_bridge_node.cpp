  ...
  class ArduinoBridgeNode : public rclcpp::Node {
   public:
    ArduinoBridgeNode() : rclcpp::Node("arduino_bridge") {
      ...
      throttle_slew_per_s_ = declare_parameter<double>("throttle_slew_per_s", 2.0);
      ...
      if (tx_rate_hz_ < 10.0) {
        RCLCPP_WARN(get_logger(),
                    "tx_rate_hz %.1f is below the Arduino's 200 ms watchdog margin, clamping to 10 Hz",
                    tx_rate_hz_);
        tx_rate_hz_ = 10.0;
      }
      ...
      command_subscription_ = create_subscription<cfr_interfaces::msg::DriveCommand>(
          ...
          [this](const cfr_interfaces::msg::DriveCommand::SharedPtr msg) { OnDriveCommand(*msg); });
      ...
      RCLCPP_INFO(get_logger(),
                  "arduino_bridge starting: device=%s baud=%u tx_rate=%.1f Hz max_throttle=%.2f",
                  device_.c_str(),
                  baud_,
                  tx_rate_hz_,
                  max_throttle_);
   ...
   private:
    ...
    void SendCommand(const rclcpp::Time& stamp, bool link_ok) {
      ...
      if (command_fresh && !estopped) {
        ...
        if (auto_active || !require_auto_active_) {
          ...
          throttle = std::clamp(static_cast<double>(last_command_.throttle), -1.0, 1.0) * max_throttle_;
        }
      } else if (command_fresh && estopped) {
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "E-Stop asserted, holding neutral");
      } else if (HaveCommand()) {
        RCLCPP_WARN_THROTTLE(
            get_logger(), *get_clock(), 2000, "drive command stale (> %.2f s), holding neutral", command_timeout_);
      }
    ...
    double ApplyThrottleSlew(const rclcpp::Time& stamp, double target, bool passthrough) {
      ...
      if (!passthrough) {
        ...
        commanded_throttle_ += std::clamp(target - commanded_throttle_, -max_step, max_step);
      }
