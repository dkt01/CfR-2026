...
isolation.
    ...
    python3 tui.py --dir rl/obstacleRacer/runs/v1   # an obstacle-racer run

A directory holding an rl/obstacleRacer history.json is shown by
racer_view.py instead: that trainer runs in numpy with no container and no
chunk logs, and records every evaluation in the file itself.
...
def main():
    ...
    racer = racer_view.is_racer_run(checkpoint_dir)
    try:
        while True:
            if racer:
                frame = racer_view.render(
                    checkpoint_dir,
